            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <span class="badge bg-{{ RoomReservationStatus::tryFrom($roomReservation->status)?->color() }}">
                        {{ ucfirst(RoomReservationStatus::tryFrom($roomReservation->status)?->label()) }}
                    </span>
                                    </td>
                                        ...
                                        <div class="d-flex justify-content-center align-items-center gap-3 py-1">
                                            @if(!$roomReservation->checked_in_at)
                                                <button type="button"
                                                        ...
                                                        data-bs-room-reservation="{{ $roomReservation->id }}">
                                                    Check In
                                                </button>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            if (checkInModal) {
                checkInModal.addEventListener('show.bs.modal', function (event) {
                    ...
                    const roomReservationId = button.getAttribute('data-bs-room-reservation');

                    const form = document.getElementById('checkInForm');
                    const checkInBaseUrl = "{{ url('admin/roomReservations') }}";

                    form.action = `${checkInBaseUrl}/${roomReservationId}/check-in`;
                });
            }
            ...
            if (checkOutModal) {
                checkOutModal.addEventListener('show.bs.modal', function (event) {
                    ...
                    const checkOutBaseUrl = "{{ url('admin/roomReservations') }}";

                    form.action = `${checkOutBaseUrl}/${roomReservationId}/check-out`;
                });
            }
